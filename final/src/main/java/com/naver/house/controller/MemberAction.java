...
public class MemberAction {
	...
	public ModelAndView member_login_ok(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		/*
		 * MemberBean m = this.memberService.userCheck(id); AdminBean ad=
		 * this.memberService.adminCheck(id);
		 */
		...
		if (this.memberService.admin_idcheck(id) != 0) {
			...
			if (ad.getAdmin_pwd().equals(pwd)) {// 비번이 같을 때
				...
				session.setAttribute("join_name", "관리자");
				session.setAttribute("Admin_no", ad.getAdmin_no());
				session.setAttribute("auto_no", 1);
			...
			if (m.getPassword().equals(pwd)) {// 비번이 같을 때
				...
				session.setAttribute("auto_no", 0);
		...
		if (this.memberService.userCheck(id) == null && this.memberService.adminCheck(id) == null) {// 등록되지 않은 회원일 때(
																									// id로 member, admin
																									// table 조회 시 없음 )
			out.println("<script>");
	...
	public ModelAndView memberinfo(HttpServletRequest request, ModelAndView mv, @RequestParam(value = "id") String id)
			throws Exception {
		// 아이디 키값의 세션 아이디를 구함
