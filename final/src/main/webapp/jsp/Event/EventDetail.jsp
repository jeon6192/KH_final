...
<script>
	...
	function start(){
		...
		var complex_id = $('#complex_id').val();
		var apt_listcount = $('#apt_listcount').val();
		...
		if(confirm("제비뽑기"+length1+"명  점수 순위 뽑기 "+length2+"명 입니다. 분양을 시작하시겠습니까?")){
			length1=length1*1;
			length2=length2*1;
			apt_listcount=apt_listcount*1;
			
			if((length1+length2)<=apt_listcount){
		...
		if(confirm("미분양중인 "+total+"세대에 대하여 재분양을 시작하시겠습니까?")){
			...
			py=(sh-ch)/2;
				...
				$.ajax({
					...
					success:function(data){
							alert("재분양을 시작합니다.");
							window.open("restartform.ev?complex_id="+complex_id,"재분양 설정","width=800,height=800");
							
							/* window.open("restartform.ev?complex_id="+complex_id,"재분양 설정 ",'left='+px+',top='+py+',width=600,height=550,toolbar=no,menubar=no,status=no,scrollbars=no,resizable=no'); */
				...
				<div class="detail cpx">
					...
					<div class="cpx info">
						...
						<div class="div date">
							...
							<div class="cpx pdate">
								<input type="hidden" value="${apt_listcount}" id="apt_listcount">
								${apt_listcount } 세대
							</div>
						</div>
